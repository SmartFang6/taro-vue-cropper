    ...
    <view>
      ...
      <view class="cropper-config">
        <view class="button-box">
          <button type="warn" @tap="chooseImage">重选</button>
          <button type="warn" @tap="previewImg">预览</button>
          <button type="warn" @tap="finish">完成</button>
        </view>
      </view>
      <canvas
        canvas-id="myCanvas"
  ...
  methods: {
    ...
    loadImage: function () {
      ...
      Taro.getImageInfo({
        ...
        success: function success(res) {
          ...
          if (
            IMG_RATIO < 1 &&
            (SCREEN_HEIGHT - 120) * IMG_RATIO < SCREEN_WIDTH - 10
          ) {
          ...
          /* 初始化裁剪区域的位置和形状  */
          let [cutT, cutB, cutL, cutR] = _this.initCutArea(
            IMG_RATIO,
            IMG_REAL_H,
            IMG_REAL_W
          );
    ...
    previewImg() {
      try {
        ...
        // 将图片写入画布
        const ctx = Taro.createCanvasContext("myCanvas", _this);
    ...
    fixedDrag(e) {
      var _this = this;
      var dragType = this.$refs.drag.dataset.drag;
...
.uni-corpper-crop-box .uni-cropper-view-box {
  ...
  max-height: calc(100vh - 100px);
  outline: 5px solid rgb(100, 97, 97);
